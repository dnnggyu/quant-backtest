...
Ver3.0~3.7: (이전 변경 이력 생략)
Ver3.8: 모델별 분석 시각화 + 피처 확장
        WilliamsR_14 제거 (Stoch_K와 수학적 동치)
        신규 피처 8개: RS_Market(상대강도), Beta, Rolling Sharpe, Win Rate, Vol Ratio → 총 55개
        앙상블 방식: raw 평균 → Rank Average 전환 (스케일 무관, 순위 기반 합의)
        모델별 피처 중요도 비교 차트 (RF vs XGB vs LGBM, 정규화 스케일)
        모델별 IC 추이 비교 + 모델 합의도 (★★★) 표시
        각 탭에 AI 모델 설명 추가
        생존자 편향 퇴출 종목 섹터 필터 수정
"""
...
def main():
    st.markdown('<div class="main-title">📊 AI Quant Lab 3.8</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="sub-title">AI 퀀트 백테스팅 & 실시간 추천 · '
        '55개 피처 (상대강도·베타·롤링Sharpe 포함) · RF+XGB+LGBM 앙상블 · '
        '모델별 IC·중요도 비교 · 합의도(★) · PIT 22개 지표 · 생존자 편향 보정</div>',
        unsafe_allow_html=True,
    )
    ...
    for k in ["results", "benchmarks", "price_data", "fund_map", "tech_map",
              "cfg", "rf_rate", "pit_map", "sp500_changes", "spy_close"]:
    ...
    if cfg["run"]:
        ...
        # 3. SPY 다운로드 (상대강도·베타 계산용) + 기술지표 사전 계산
        update_prog(0.28, "📡 SPY 벤치마크 다운로드 + 기술지표 계산 중...")
        spy_close = None
        try:
        ...
        for t, ohlcv in price_data.items():
            try:
                ...
                pass
        update_prog(0.30, f"📈 기술지표 계산 완료 ({len(tech_map)}종목, SPY 상대강도 포함). 백테스트 시작...")
