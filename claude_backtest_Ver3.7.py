...
Ver3.0~3.6: (이전 변경 이력 생략)
Ver3.7: 앙상블 모델
        RF + XGBoost + LightGBM 3모델 단순 평균 앙상블
        UI 토글로 앙상블/단일 RF 전환 가능
        각 모델별 피처 중요도 비교 시각화
"""
...
def run_backtest(
    ...
    """메인 백테스트 엔진.
    Ver3.7: RF+XGBoost+LightGBM 앙상블 (use_ensemble=True).
    """
    ...
    for i in range(rolling_win, n_dates - 1):
        ...
        # 중요도: 3모델 평균 (원본 피처만, 결측 플래그 제외)
        imp_dict = {}
        for idx_c, col_name in enumerate(all_train_cols):
            if col_name in avail_cols:
                rf_imp = model_rf.feature_importances_[idx_c]
                if use_ensemble:
                    xgb_imp  = model_xgb.feature_importances_[idx_c]
                    lgbm_imp = model_lgbm.feature_importances_[idx_c]
                    imp_dict[col_name] = (rf_imp + xgb_imp + lgbm_imp) / 3
                else:
                    imp_dict[col_name] = rf_imp
        ...
        # 앙상블 예측: 3모델 단순 평균
        pred_rf = model_rf.predict(X_pred_imp)
        if use_ensemble:
            pred_xgb  = model_xgb.predict(X_pred_imp)
            pred_lgbm = model_lgbm.predict(X_pred_imp)
            pred_returns = (pred_rf + pred_xgb + pred_lgbm) / 3
        else:
            pred_returns = pred_rf
...
def tab_realtime(price_data: dict, fund_map: dict, tech_map: dict,
    ...
    if is_ensemble and model_xgb is not None and model_lgbm is not None:
        ...
        pred_lgbm = model_lgbm.predict(X_pred_imp)
        pred_returns = (pred_rf + pred_xgb + pred_lgbm) / 3
        st.caption("🤖 앙상블 예측 (RF + XGBoost + LightGBM 평균)")
    else:
        pred_returns = pred_rf
        st.caption("🤖 단일 RF 예측")
...
def main():
    st.markdown('<div class="main-title">📊 AI Quant Lab 3.7</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="sub-title">AI 퀀트 백테스팅 & 실시간 추천 · '
        'RF+XGBoost+LightGBM 앙상블 · '
        'S&P 1500 · 생존자 편향 보정 · PIT 22개 지표 · Look-Ahead 제거</div>',
        unsafe_allow_html=True,
    )
